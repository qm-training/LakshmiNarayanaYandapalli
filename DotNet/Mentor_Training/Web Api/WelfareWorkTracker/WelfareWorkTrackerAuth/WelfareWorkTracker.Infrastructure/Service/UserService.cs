                            ...
                            IAuthService authService) : IUserService
{
    ...
    public async Task<bool> RegisterAsync(RegisterUserVm registerUserVm)
    {
        ...
        var userRegistered = await _userRepository.RegisterUserAsync(user);

        // Send Welcome Email
        var emailTemplate = await _emailTemplateRepository.GetByNameAsync(Constants.EmailTemplates.WelcomeEmail);
        if (userRegistered)
        {
            ...
            Log.Information("Registration email sent successfully to {UserEmail}", user.Email);
        }
    ...
    public async Task<bool> ResetPasswordAsync(ResetPasswordVm resetPasswordVm)
    {
        ...
        var emailTemplate = await _emailTemplateRepository.GetByNameAsync(Constants.EmailTemplates.PasswordResetSuccessful);
        if (updatedUser)
        {
            var payload = new Dictionary<string, string>
            {
            ...
            Log.Information("Password Reset confirmation email sent successfully to {UserEmail}", user.Email);
        }
